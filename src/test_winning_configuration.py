...
# create an argparser for the below 
parser = argparse.ArgumentParser()
parser.add_argument('--domain', type=str, default='LSVM', help='Domain to train on') # options: 'LSVM', 'GSVM', 'SRVM', 'MRVM'
parser.add_argument('--bidder_type', type=str, default='National', help='Bidder type to train on') # options: 'National', 'Regional', 'High_frequency', 'Local'
parser.add_argument('--training', type=str, default='mixed', help='Training type to use') # options: 'mixed', 'dqs', 'vqs'
parser.add_argument('--optimization_metric', type=str, default='dqs_r2', help='Optimization metric to use') # options: 'vqs_r2', 'dqs_r2
...
# training = 'mixed'  # options: 'mixed', 'dqs', 'vqs'
# optimization_metric = 'dqs_r2' # options: 'vqs_r2', 'dqs_r2

instances_to_average = 5   # TODO: change back to 10 # number of instances to average over
...
def bidder_type_to_first_bidder_id(SATS_domain,
                             bidder_type):   # to make the results comparable, now we always plot the first bidder
    ...
    bidder_id = bidder_id_mappings[SATS_domain][bidder_type.lower()][0]
...
def main(): 
    ...
    # Load best configurations from external JSON file
    with open("winning_configurations_all_training_types.json", "r") as f:
        best_configs = json.load(f)

    # Load the best configuration for the current domain
    config = best_configs[training][optimization_metric][domain][bidder_type]
    ...
    if training == 'mixed':
        ...
        raise ValueError('Training type not recognized')

    # 1. SATS Parameters (plus test paramters)
    # ----------------------------------------------
    SATS_parameters = {"SATS_domain": domain,
                    "isLegacy": False,
                    "SATS_seed": 1 + seed_offset,
    ...
    # ----------------------------------------------
    TRAIN_parameters_single_bidder = {"number_train_data_points": dqs,  # used to be 40 for the main experiments
                        "number_train_data_points_cardinal": vqs,   # used to be 20 for the main experiments 
                        ...
                        "max_linear_prices_multiplier": config['max_linear_prices_multiplier'],
                        ...
                        'scale_multiplier': 1,  # only used with dynamic scaling
                        'print_frequency': 1, 
    ...
    # ----------------------------------------------
    if wandb_tracking:
        run = wandb.init(project=f'Winning-configuration-test-instances-v1.1', 
    ...
    for i in range(instances_to_average):
        ...
        bidder_id = bidder_type_to_first_bidder_id(SATS_domain= SATS_parameters['SATS_domain'], bidder_type=bidder_type)
    ...
    for epoch in range(epochs):
        ...
        if avg_metrics_val_dqs is not None:
            ...
            if avg_metrics_val_dqs.get('r2_train', False): 
                new_dict = {
    ...
    true_values_train = metrics_single_run_last_epoch['val_metrics_demand_queries'].get('scaled_true_values_train', np.zeros(100))
    predicted_values_train = metrics_single_run_last_epoch['val_metrics_demand_queries'].get('scaled_predicted_values_train', np.zeros(100))
    inferred_values_train = metrics_single_run_last_epoch['val_metrics_demand_queries'].get('scaled_inferred_values_train', np.zeros(100))

    # true_values_train = metrics_single_run_last_epoch['val_metrics_demand_queries']['scaled_true_values_train'] # --- OLD VERSION THAT DID NOT HANDLE THAT --- # 
    ...
    # Predictive performance on the VQ train dataset, if it exists 
    true_values_train_vqs = metrics_single_run_last_epoch.get('train_metrics_vqs', {'true_values_train': np.zeros(100)})['true_values_train'].flatten()   # NEW TO HANDLE THE NONE CASE 
    predicted_values_train_vqs = metrics_single_run_last_epoch.get('train_metrics_vqs', {'predicted_values_train': np.zeros(100)})['predicted_values_train'].flatten()
    ...
    plt.figure(figsize =(9,9))
    # plt.scatter(true_values_train, inferred_values_train, s = 30, alpha = 1, marker = 'v',facecolors='none', edgecolors='lightgreen', label = 'I.V. for DQ trainset')
    plt.scatter(true_values_PV, inferred_values_PV,  s = 30, alpha = 0.8,facecolors='none', edgecolors='lightskyblue', marker='v', label = 'I.V. for test set $\mathcal{T}_p$')
    # plt.scatter(true_values_train, predicted_values_train,  s = 30, alpha = 1,facecolors='none', edgecolors='darkgreen', label = 'NN P. for DQ trainset')
    plt.scatter(true_values_PV, predicted_values_PV, s = 30, alpha = 0.8,facecolors='none', edgecolors='royalblue', label = 'NN P. for test set $\mathcal{T}_p$')
    
    plt.scatter(true_values, predicted_values, c='crimson', s = 0.4, alpha = 0.4,facecolors='none', edgecolors='lightcoral', label = 'NN P. for test set $\mathcal{T}_r$')
    # plt.scatter(true_values_train_vqs, predicted_values_train_vqs, s = 30, alpha = 1,facecolors='none', c = 'plum',edgecolors='plum', label = 'NN P. for VQ trainset')
