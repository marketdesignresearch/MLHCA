...
import os
# Own Libs
from mlca_demand_queries.mlca_hybrid_economies import MLCA_HYBRID_ECONOMIES
...
# %% MECHANISM
def hybrid_mechanism(SATS_parameters: dict,
    ...
    mlcca_dqs = mechanism_parameters['MLCCA-DQs']
    mlcca_vqs = mechanism_parameters['MLCCA-VQs']
    ...
    value_query_method = mechanism_parameters['value_query_method']
    ...
    logging.warning(f'Total Queries: {mlcca_dqs + mlcca_vqs+ Qinit}')
    ...
    logging.warning(f'value_query_method: {value_query_method}')
    logging.warning('')
    ...
    elif SATS_domain in ['MRVM', 'MRVM_non_generic']:
    ...
    elif SATS_domain in ['SRVM', 'SRVM_non_generic']:
    ...
    E = MLCA_HYBRID_ECONOMIES(SATS_auction_instance = SATS_auction_instance,
                          SATS_parameters = SATS_parameters,
                          TRAIN_parameters = TRAIN_parameters,
                          MVNN_parameters= MVNN_parameters,
                          TRAIN_parameters_dqs = TRAIN_parameters_dqs,
                          MVNN_parameters_dqs = MVNN_parameters_dqs,
                          mechanism_parameters = mechanism_parameters,
                          start_time=start, 
    ...
    # Global while loop: check if for all bidders one additional auction round is feasible
    for iteration in range(1, (E.mlcca_dqs + E.mlcca_vqs) + 1):
