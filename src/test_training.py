...
# own libs
from mlca_demand_queries.mlca_dq_util import init_demand_queries_mlca_unif, init_demand_queries_mlca_cca, key_to_int, initial_bids_mlca_unif
from mvnns_demand_query_training.mvnn_dq_training import dq_train_mvnn, vq_train_mvnn
...
def prepare_training_data(SATS_parameters,
                                    TRAIN_parameters,
                                    init_method, 
                                    ...
                                    bidder_id):
    """
    A function that does all the preparatory things in order to invoke train_mvnn_parallel from the HPO script 
    """  
    # --- Create SATS World ------ # 
    SATS_domain = SATS_parameters['SATS_domain']
    isLegacy = SATS_parameters['isLegacy']
    ...
    # print('-----Inside prepare training data', wandb_tracking)
    ...
    # add those to SATS_parameters  # TODO: check if this is necessary/ who uses that data 
    TRAIN_parameters['max_linear_prices'] = max_linear_prices
    TRAIN_parameters['scale'] = scales[f'Bidder_{bidder_id}']
    SATS_parameters['GSVM_national_bidder_goods_of_interest'] = GSVM_national_bidder_goods_of_interest
    ...
    if init_method == 'random':
        ...
        final_prices = DQdata_train_only[1][-1]
        print('Done preparing DQ train data')
        ...
        print('Done preparing DQ validation data')
        
        #  combine the data so that they have the same format as in the case where the method is random. 
        train_DQdata = {f'Bidder_{bidder_id}': [np.concatenate((DQdata_train_only[0], DQdata_val_only[f'Bidder_{bidder_id}'][0])), np.concatenate((DQdata_train_only[1], DQdata_val_only[f'Bidder_{bidder_id}'][1]))]} 

    
    # --- Create Initial VQ Training and Validation Data --- # 
    if init_method_vq == 'random':
        # train_VQdata = init_demand_queries_mlca_unif(SATS_auction_instance,
        #                                             number_initial_bids = TRAIN_parameters['number_train_data_points'] + TRAIN_parameters['number_val_data_points'],
        #                                             max_linear_prices = {k:TRAIN_parameters['max_linear_prices_multiplier']*v for k,v in max_linear_prices.items()},
        #                                             seed = TRAIN_parameters['data_seed'], 
        #                                             bidder_id = bidder_id 
        ...
        # train_VQdata = initial_bids_mlca_unif(SATS_auction_instance,
        #                                         number_initial_bids = TRAIN_parameters['number_train_data_points_cardinal'] + TRAIN_parameters['number_val_data_points_cardinal'],
        #                                         bidder_names= ['bidder' + str(bidder_id)], 
        #                                         scaler= None, 
        #                                         seed = TRAIN_parameters['data_seed'], 
        ...
        before = time.time()
        bundles_gen = generate_random_bundles(capacities=good_capacities, number_of_bundles=TRAIN_parameters['number_train_data_points_cardinal'] + TRAIN_parameters['number_val_data_points_cardinal'])
        
        true_values_generalization = np.array([SATS_auction_instance.calculate_value(bidder_id, bundle) for bundle in bundles_gen])
        after = time.time()
        print(f'Generating {bundles_gen.shape} bundles took {after - before} seconds')
        

        print('Done preparing VQ train data')
        
    else: 
        raise ValueError(f'init_method_vq:{init_method_vq} not implemented yet.')
    
    

    # --- Convert the dataset to a more readable format --- # 
    train_DQdata_X = train_DQdata[f'Bidder_{bidder_id}'][0][:TRAIN_parameters['number_train_data_points']]
    train_DQdata_P = train_DQdata[f'Bidder_{bidder_id}'][1][:TRAIN_parameters['number_train_data_points']]
    val_DQdata_X = train_DQdata[f'Bidder_{bidder_id}'][0][TRAIN_parameters['number_train_data_points']:]
    val_DQdata_P = train_DQdata[f'Bidder_{bidder_id}'][1][TRAIN_parameters['number_train_data_points']:]

    training_set_DQs = {}
    val_set_DQs = {} 
    training_set_DQs[f'Bidder_{bidder_id}'] = (train_DQdata_X, train_DQdata_P)
    val_set_DQs[f'Bidder_{bidder_id}'] = (val_DQdata_X, val_DQdata_P)

    train_VQdata_X = bundles_gen[:TRAIN_parameters['number_train_data_points_cardinal']]    
    train_VQdata_Y = true_values_generalization[:TRAIN_parameters['number_train_data_points_cardinal']]
    val_VQdata_X = bundles_gen[TRAIN_parameters['number_train_data_points_cardinal']:]
    val_VQdata_Y = true_values_generalization[TRAIN_parameters['number_train_data_points_cardinal']:]    

    training_set_VQs = {}
    val_set_VQs = {}
    training_set_VQs[f'Bidder_{bidder_id}'] = (train_VQdata_X, train_VQdata_Y)
    val_set_VQs[f'Bidder_{bidder_id}'] = (val_VQdata_X, val_VQdata_Y)
    

    print('---> Inside prepare data,  train_VQdata finished')
    
    return  (good_capacities, training_set_DQs, val_set_DQs, training_set_VQs, 
                val_set_VQs, scales, GSVM_national_bidder_goods_of_interest, TRAIN_parameters, SATS_parameters, SATS_auction_instance)
