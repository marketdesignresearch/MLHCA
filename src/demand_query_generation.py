...
def extend_price(generic_price_vector, capacities_generic_goods, good_to_license_mapping): 
    """
    ...
    price_extended = [0 for i in range(item_number)]
    for i in range(len(generic_price_vector)): 
        identical_items = good_to_license_mapping[i]
        for j in identical_items:
            price_extended[j] = generic_price_vector[i]
    
    price_extended = np.array(price_extended)

    return price_extended

def compress_demand(extended_demand, capacities_generic_goods, inverse_mapping): 
    """
    A simple function that compresses a demand vector from the naive form to the generic form. 
    """
    # set_trace()
    compressed_demand = [0 for i in range(len(capacities_generic_goods))]
    for i in range(len(extended_demand)): 
        compressed_demand[inverse_mapping[i]] += extended_demand[i]

    # set_trace()
    return compressed_demand
...
def minimize_W_v3_non_generic(bidder_models,
            starting_price_vector,
            capacities,
            scale,
            SATS_domain,
    ...
    """
    A minimize W version that works only with non generic models
    bidder_models: list of tuples (bidder_id, bidder_model)
    starting_price_vector: np.array of initial prices
    capacities: list of capacities for each good
    scale: list of scale factors for each bidder model
    ...
    solver_models = []
    number_of_bidders = len(bidder_models)
    inverse_mapping = {value: key for key, value_list in good_to_license_mapping.items() for value in value_list}

    starting_price_vector_extended = extend_price(starting_price_vector, capacities_generic_goods = capacities, good_to_license_mapping = good_to_license_mapping)
    for (bidder_id, bidder_model) in bidder_models:
        bidder_model.transform_weights()
        bidder_model.eval()
        ...
        print(starting_price_vector)
        solver.generate_mip(starting_price_vector_extended)
        solver_models.append(solver)

    # Start minimizing W 
    price_vector = starting_price_vector
    price_vector_extended = extend_price(price_vector, capacities_generic_goods = capacities, good_to_license_mapping = good_to_license_mapping)
    best_price_vector = starting_price_vector 
    best_W = np.inf
    best_price_vector_is_feasible = False
    ...
    while (steps_without_improvement < max_steps_without_improvement) and (all_steps < max_steps):
        total_predicted_demand_extended = np.zeros(price_vector_extended.shape[0])
        indirect_revenue = np.dot(price_vector, capacities)
        current_W = indirect_revenue
        ...
        for (i, solver) in enumerate(solver_models):
            # Solve the MIP for the current price vector
            # Multiply the price vector by the scale factor because all mvnns are scaled to have outputs in [0,1]
            price_vector_scaled_extended = price_vector_extended / scale[i]
            solver.update_prices_in_objective(price_vector_scaled_extended)
            
            try:
                predicted_demand_extended = np.array(solver.solve_mip(outputFlag=False,
                                                        verbose = False,
                                                        timeLimit = MIP_parameters["timeLimit"],
                                                        MIPGap = MIP_parameters["MIPGap"],
                                                        IntFeasTol = MIP_parameters["IntFeasTol"],
                ...
                print('Error in MIP solution, skipping this agent') 
                current_W = 2 * current_W # penalize the crushed agent by doubling its W
                
            
            total_predicted_demand_extended += predicted_demand_extended  # add the predicted demand to the total predicted demand


        total_predicted_demand = compress_demand(total_predicted_demand_extended, capacities_generic_goods= capacities, inverse_mapping= inverse_mapping)
        over_demand = total_predicted_demand - capacities
        ...
        # Calculate the extended representation of the price vector
        price_vector_extended = extend_price(price_vector, capacities_generic_goods = capacities, good_to_license_mapping = good_to_license_mapping)
